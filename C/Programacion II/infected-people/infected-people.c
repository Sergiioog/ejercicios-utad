	...
	*/
		...
		while(1){
			...
			switch(opcion){
				...
				case 5:
					...
					liberaCiudad(ciudad, tamanoCiudad);
					liberaHistorial(&historialInfectados);
					return 0;
				default:
	...
	char** crearCiudad(int n){
		...
		char **ciudad = (char**)malloc(sizeof(char*)* (n + 1));
	...
	void muestraHistorial(historial_t* historial){
		...
		for(int i = 0; i < historial->cantidad; i++){
			printf("[Fila: %d / Columna: %d / Dia: %d]\n", historial->histInfectados[i].fila, historial->histInfectados[i].columna, historial->histInfectados[i].dia);
		}
	...
	void avanzarDia(char** ciudad, int n, historial_t* historial, int diaActual){
		...
		for(int i = 0; i < n; i++){
			for(int j = 0; j < n; j++){
				matrizAuxiliar[i][j] = 'S'; //Rellenamos la matriz auxiliar con personas sanas
			}
		}
		...
		for(int i = 0; i < n; i++){
			for(int j = 0; j < n; j++){
				if(ciudad[i][j] == 'I'){
					//2. Todas las personas que estaban infectadas ('I') pasaran a recuperadas ('R') (volvemos a recorrer la matriz, porque si no alterariamos la logica de la primera iteración de infectados)
					ciudad[i][j] = 'R';
				}
			}
		}
